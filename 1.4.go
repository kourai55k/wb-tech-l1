...
//Для завершения работы воркеров выбран контекст (context.Context) по следующим причинам:
//1. Плaвное завершение: Воркеры завершаются мягко, обрабатывая текущие задачи и корректно выходя из цикла.
//2. Централизованное управление: Один вызов cancel() отправляет сигнал завершения всем воркерам одновременно.
